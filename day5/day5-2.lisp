...
(defun operate (p op l v x y)
  (cond ((= op 1) (store (+ p 4) (+ (immediatecheck l v x 1) (immediatecheck l v y 2)) (CADDDR v) l))
        ...
        ((= op 5) (jumpt (+ p 3) (immediatecheck l v x 1) (immediatecheck l v y 2) l))
        ((= op 6) (jumpf (+ p 3) (immediatecheck l v x 1) (immediatecheck l v y 2) l))
        ((= op 7) (store (+ p 4) (check `< (immediatecheck l v x 1) (immediatecheck l v y 2)) (CADDDR v) l))
        ((= op 8) (store (+ p 4) (check `= (immediatecheck l v x 1) (immediatecheck l v y 2)) (CADDDR v) l))
...
(defun jumpt (p x y l)
  (if (= 0 x) (intcode p l) (intcode y l))
)

(defun jumpf (p x y l)
  (if (= 0 x) (intcode y l) (intcode p l))
)
