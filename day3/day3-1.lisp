...
(defun coord (path l)
  (if (null path) l
    (cond ((eql (char (CAR path) 0) #\L)
          (coord (CDR path) (append l (list (CONS (- (CAAR (last l 1)) (parse-integer (string-left-trim "L" (CAR path)))) (CDAR (last l 1)))))))
          ((eql (char (CAR path) 0) #\R)
          (coord (CDR path) (append l (list (CONS (+ (CAAR (last l 1)) (parse-integer (string-left-trim "R" (CAR path)))) (CDAR (last l 1)))))))
          ((eql (char (CAR path) 0) #\U)
          (coord (CDR path) (append l (list (CONS (CAAR (last l 1)) (+ (CDAR (last l 1)) (parse-integer (string-left-trim "U" (CAR path)))))))))
          ((eql (char (CAR path) 0) #\D)
          (coord (CDR path) (append l (list (CONS (CAAR (last l 1)) (- (CDAR (last l 1)) (parse-integer (string-left-trim "D" (CAR path)))))))))
    )
  )
)
